...
class WarCommands(commands.GroupCog, name="war"):
    ...
    @app_commands.command(
        name="action",
        description="⚔️ THE PLAYER COMMAND - Submit your combat action! Use /help action for tutorial.",
    )
    @app_commands.guild_only()
    @app_commands.choices(main=[
        app_commands.Choice(name="Attack", value="attack"),
        app_commands.Choice(name="Defend (+2 bonus)", value="defend"),
    ...
    @app_commands.describe(
        war_id="War ID (autocomplete shows your active wars)",
        main="Main action: Attack (standard) | Defend (+2 to roll) | Super Unit (if researched)",
        minor="Minor action: Prepare Attack (+1 next) | Sabotage (-1 enemy) | Fortify (+1 next defense) | Heal (recover HP) | Prepare Super Unit",
        narrative_link="Discord message link to your RP narrative (write 100+ chars in RP channel FIRST, then right-click → Copy Message Link)",
        roll="Your d20 roll result (YOU roll it yourself using /roll 1d20 or physical dice - the bot does NOT roll for you!)"
    )
